...
class that implements a priority queue, but this component has a "strange"
...
this can be a problem in many situations. Even, though, the [definition](https://en.wikipedia.org/wiki/Priority_queue)
...
I did not use the usual approach to implement the priority queue with a [heap](https://en.wikipedia.org/wiki/Heap_%28data_structure%29).
I've used [ordered arrays](https://github.com/ezimuel/FastPriorityQueue/blob/master/src/PriorityQueue.php#L19)
...
I have also included the `SplPriorityQueue` of PHP to have a starter point for the
...
As you can see the execution time of `FastPriorityQueue` is very good, about **3x
to 6x faster** than the other implementations (except the `SplPriorityQueue` that
is out of the comparison).

## Unit Tests

You can run the unit tests by executing the following commmand after the installation
